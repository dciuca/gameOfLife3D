...
void RaylibRenderer::renderGrid()
{
    ...
    float cameraPos[3] = {m_camera.getCamera().position.x, m_camera.getCamera().position.y, m_camera.getCamera().position.z};
...
void RaylibRenderer::drawStats(bool paused, double computeMs, double drawMs, int fps)
{
    ...
    std::string line2 = "TARGET FPS: " + std::to_string(fps) + " | draw: " + std::to_string(drawMs) + " ms";
...
void RaylibRenderer::initInstancedShader()
{
    ...
    float lightPos[3] = {m_camera.getCamera().position.x, m_camera.getCamera().position.y + 20.0f, m_camera.getCamera().position.z};
